...
final class TaskProgressParams private (
  ...
  override def equals(o: Any): Boolean = this.eq(o.asInstanceOf[AnyRef]) || (o match {
    case x: TaskProgressParams => (this.taskId == x.taskId) && (this.originId == x.originId) && (this.eventTime == x.eventTime) && (this.message == x.message) && (this.total == x.total) && (this.progress == x.progress) && (this.unit == x.unit) && (this.dataKind == x.dataKind) && (this.data == x.data)
    case _ => false
  })
  override def hashCode: Int = {
    37 * (37 * (37 * (37 * (37 * (37 * (37 * (37 * (37 * (37 * (17 + "sbt.internal.bsp.TaskProgressParams".##) + taskId.##) + originId.##) + eventTime.##) + message.##) + total.##) + progress.##) + unit.##) + dataKind.##) + data.##)
  }
  override def toString: String = {
    "TaskProgressParams(" + taskId + ", " + originId + ", " + eventTime + ", " + message + ", " + total + ", " + progress + ", " + unit + ", " + dataKind + ", " + data + ")"
  }
  private[this] def copy(taskId: sbt.internal.bsp.TaskId = taskId, originId: Option[String] = originId, eventTime: Option[Long] = eventTime, message: Option[String] = message, total: Option[Long] = total, progress: Option[Long] = progress, unit: Option[String] = unit, dataKind: Option[String] = dataKind, data: Option[sjsonnew.shaded.scalajson.ast.unsafe.JValue] = data): TaskProgressParams = {
    new TaskProgressParams(taskId, originId, eventTime, message, total, progress, unit, dataKind, data)
  }
  def withTaskId(taskId: sbt.internal.bsp.TaskId): TaskProgressParams = {
    copy(taskId = taskId)
  }
  def withOriginId(originId: Option[String]): TaskProgressParams = {
    copy(originId = originId)
  }
  def withOriginId(originId: String): TaskProgressParams = {
...
object TaskProgressParams {
  ...
  def apply(taskId: sbt.internal.bsp.TaskId, originId: Option[String], eventTime: Option[Long], message: Option[String], total: Option[Long], progress: Option[Long], unit: Option[String], dataKind: Option[String], data: Option[sjsonnew.shaded.scalajson.ast.unsafe.JValue]): TaskProgressParams = new TaskProgressParams(taskId, originId, eventTime, message, total, progress, unit, dataKind, data)
  def apply(taskId: sbt.internal.bsp.TaskId, originId: String, eventTime: Long, message: String, total: Long, progress: Long, unit: String, dataKind: String, data: sjsonnew.shaded.scalajson.ast.unsafe.JValue): TaskProgressParams = new TaskProgressParams(taskId, Option(originId), Option(eventTime), Option(message), Option(total), Option(progress), Option(unit), Option(dataKind), Option(data))
}
