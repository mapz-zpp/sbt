...
class ManagedLogger(
  ...
  override def trace(t: => Throwable): Unit =
    logEvent(
      Level.Error,
      TraceEvent("Error", t, channelName, execId, Some("ManagedLogger::trace/1"))
    )
  override def log(level: Level.Value, message: => String): Unit =
    xlogger.log(level, message, originId.getOrElse("?ManagedLogger"))
  ...
  def logEvent[A: JsonFormat](level: Level.Value, event: => A)(
    ...
    xlogger.log(level, entry, "ManagedLogger::logEvent/2")
  }
